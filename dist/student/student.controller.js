...
let StudentController = class StudentController {
    ...
    async getStudentLessons(req, studentId) {
    ...
    async getUpcomingLessons(req, studentId) {
        const userId = req.user.sub || req.user.id;
        if (userId !== studentId) {
            throw new Error('Não autorizado');
        }
    ...
    async getPastLessons(req, studentId) {
        const userId = req.user.sub || req.user.id;
        if (userId !== studentId) {
            throw new Error('Não autorizado');
        }
    ...
    async getStudentPayments(req, studentId) {
        const userId = req.user.sub || req.user.id;
        if (userId !== studentId) {
            throw new Error('Não autorizado');
        }
    ...
    async getPaymentSummary(req, studentId) {
        const userId = req.user.sub || req.user.id;
        if (userId !== studentId) {
            throw new Error('Não autorizado');
        }
